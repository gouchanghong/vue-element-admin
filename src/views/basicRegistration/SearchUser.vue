<template>
  <div style="display: inline;">
    ...
    <div class="app-container searchUserDiv">
      <div class="filter-container">
        <status-radio-check ref="searchUserStatus" style="margin-left: 18px;" @getStatus="changeStatus"/>
        <span style="margin-left: 50px;">查询：</span>
        <el-input :placeholder="$t('basicRegistration.searchUserPlaceholder')" v-model="condition" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      ...
      <el-table
        ...
        <el-table-column :label="$t('basicRegistration.belongdeptname')" min-width="200px" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('basicRegistration.jobnumber')" min-width="100px" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('basicRegistration.idnumber')" min-width="200px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.idnumber }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('basicRegistration.politicalstatusname')" min-width="100px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.politicalstatusname }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('basicRegistration.educationname')" min-width="100px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.educationname }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('basicRegistration.jobtypename')" min-width="130px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.jobtypename }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('basicRegistration.personnatualname')" min-width="130px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.personnatualname }}</span>
          </template>
        </el-table-column>
      ...
      <el-dialog :title="advancedQueryTitle" :visible.sync="advancedQueryFormVisible" class="searchUserAdvancedQueryDivDialog" width="800px">
...
import { findUserByPage } from '@/api/basicRegistration'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      findUserByPage(this.page, this.limit, this.cs).then(response => {
...
<style>
  ...
  .classDisable{
    ...
    .searchUserAdvancedQueryDivDialog span.el-dialog__title {
    font-size: 20px;
    color: #000033;
  }
</style>
